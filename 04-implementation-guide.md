...
4. **환경 추가**: `WEB` → 서비스 URL: `https://www.bnkcapital.co.kr` (실제 도메인으로 변경)
...
- keyword (Multiple line text, max 1000, required)
- max_articles (Whole number, default 3)
- is_active (Yes/No, default Yes)

Table 2: News_RawLog
Columns:
- article_title (Multiple line text, max 1000, required)
- article_url (Multiple line text, max 1000)
- description (Multiple lines of text, max 2000)
- published_at (Date and time, required)
- search_keyword (Multiple line text, max 1000, required)
...
## Phase 3: Flow 1 구현 — Daily News Scraper (Power Automate)
...
솔루션 `Daily News` → **+ New → Automation → Cloud flow → Scheduled**
- **이름**: `Flow1 - Daily News Scraper`
- **시작**: 내일 오전 7시
- **반복**: 1일
- **시간대**: (UTC+09:00) Seoul
...
□ 6. 바로 테스트 혹은 다음날 자동 실행 까지 대기 → 정상 수신 확인
```
...
> **중요**: 각 테이블 추가 후 **Searchable columns** 에서 위 컬럼들을 활성화
